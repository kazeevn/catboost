...
struct THonestLikelihoodMetric: public TAdditiveMetric<THonestLikelihoodMetric> {
    explicit THonestLikelihoodMetric(ELossFunction lossFunction, double border = GetDefaultClassificationBorder());
    TMetricHolder EvalSingleThread(
        const TVector<TVector<double>>& approx,
        const TVector<float>& target,
        const TVector<float>& weight,
